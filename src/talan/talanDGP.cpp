...
void TalanDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool TalanDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule TalanDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t TalanDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t TalanDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t TalanDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t TalanDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool TalanDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void TalanDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void TalanDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void TalanDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void TalanDGP::createParamsInstance(){
...
dev::Address TalanDGP::getAddressForBlock(unsigned int blockHeight){
...
void TalanDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TalanDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TalanDGP::parseStorageOneUint64(uint64_t& value){
...
void TalanDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule TalanDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
