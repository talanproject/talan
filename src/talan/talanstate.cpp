...
#include <talan/talanstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

TalanState::TalanState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = TalanState::openDB(_path + "/talanDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

TalanState::TalanState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute TalanState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, TalanTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createTalanAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            talan::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            TalanTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                TalanTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
...
std::unordered_map<dev::Address, Vin> TalanState::vins() const // temp
{
...
void TalanState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* TalanState::vin(dev::Address const& _a) const
{
    return const_cast<TalanState*>(this)->vin(_a);
}

Vin* TalanState::vin(dev::Address const& _addr)
{
...
// void TalanState::commit(CommitBehaviour _commitBehaviour)
// {
...
void TalanState::kill(dev::Address _addr)
{
...
void TalanState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void TalanState::deleteAccounts(std::set<dev::Address>& addrs){
...
void TalanState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void TalanState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void TalanState::validateTransfersWithChangeLog(){
...
void TalanState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!TalanState::addressInUse(delegationsAddress)){
        TalanState::createContract(delegationsAddress);
        TalanState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
