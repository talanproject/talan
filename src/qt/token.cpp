...
bool ToHash160(const std::string& strTalanAddress, std::string& strHash160)
{
    CTxDestination talanAddress = DecodeDestination(strTalanAddress);
    if(!IsValidDestination(talanAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&talanAddress);
    if(keyid){
...
bool ToTalanAddress(const std::string& strHash160, std::string& strTalanAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination talanAddress = keyid;
    if(IsValidDestination(talanAddress)){
        strTalanAddress = EncodeDestination(talanAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToTalanAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToTalanAddress(tokenEvent.receiver, tokenEvent.receiver);
