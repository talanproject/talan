...
#include <qt/talanversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    talanVersionChecker = new TalanVersionChecker(this);
#ifdef ENABLE_WALLET
    if(enableWallet) {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Talan address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and talan: URIs)"));
    ...
    TLRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/tlrctoken"), tr("&TLRC Tokens"), this);
    TLRCTokenAction->setStatusTip(tr("TLRC Tokens (send, receive or add Tokens in list)"));
    TLRCTokenAction->setToolTip(TLRCTokenAction->statusTip());
    TLRCTokenAction->setCheckable(true);
    TLRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(TLRCTokenAction);
    ...
    connect(TLRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(TLRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Talan addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Talan addresses"));
    ...
    openAction->setStatusTip(tr("Open a talan: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Talan command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(TLRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && talanVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(TALAN_RELEASES, TALAN_RELEASES);
            QString message(tr("New version of Talan wallet is available on the Talan source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    TLRCTokenAction->setEnabled(enabled);
...
{
    TLRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Talan network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
