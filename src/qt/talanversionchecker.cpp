...
#define paternVersion "talan-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

TalanVersionChecker::TalanVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

TalanVersionChecker::~TalanVersionChecker()
{

}

bool TalanVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> TalanVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(TALAN_RELEASES)));
    QEventLoop event;
...
Version TalanVersionChecker::getMaxReleaseVersion()
{
