#include <qt/tlrctoken.h>
#include <qt/forms/ui_tlrctoken.h>
...
TLRCToken::TLRCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TLRCToken),
    m_model(0),
    m_clientModel(0),
    m_tokenTransactionView(0)
{
    ...
    connect(m_tokenList, &TokenListWidget::sendToken, this, &TLRCToken::on_sendToken);
    connect(m_tokenList, &TokenListWidget::receiveToken, this, &TLRCToken::on_receiveToken);
    connect(m_tokenList, &TokenListWidget::addToken, this, &TLRCToken::on_addToken);
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &TLRCToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &TLRCToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &TLRCToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &TLRCToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &TLRCToken::removeToken);

    connect(m_tokenList, &TokenListWidget::customContextMenuRequested, this, &TLRCToken::contextualMenu);
}
...
void TLRCToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    {
        // Set current token
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::dataChanged, this, &TLRCToken::on_dataChanged);
        connect(m_tokenList->tokenModel(), &QAbstractItemModel::rowsInserted, this, &TLRCToken::on_rowsInserted);
...
void TLRCToken::setClientModel(ClientModel *_clientModel)
{
...
void TLRCToken::on_goToSendTokenPage()
{
    m_sendTokenPage->show();
}

void TLRCToken::on_goToReceiveTokenPage()
{
    m_receiveTokenPage->show();
}

void TLRCToken::on_goToAddTokenPage()
{
    m_addTokenPage->show();
}

void TLRCToken::on_currentTokenChanged(QModelIndex index)
{
...
void TLRCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void TLRCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void TLRCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void TLRCToken::contextualMenu(const QPoint &point)
{
...
void TLRCToken::copyTokenAddress()
{
...
void TLRCToken::copyTokenBalance()
{
    if(indexMenu.isValid())
    {
...
void TLRCToken::copySenderAddress()
{
    if(indexMenu.isValid())
    {
...
void TLRCToken::on_sendToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToSendTokenPage();
}

void TLRCToken::on_receiveToken(const QModelIndex &index)
{
    on_currentTokenChanged(index);
    on_goToReceiveTokenPage();
}

void TLRCToken::on_addToken()
{
    on_goToAddTokenPage();
}
