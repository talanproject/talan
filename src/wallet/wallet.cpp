...
bool CWallet::CreateCoinStakeFromMine(interfaces::Chain::Lock& locked_chain, const FillableSigningProvider& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, COutPoint& headerPrevout)
{
    ...
    CAmount defFundPayment = GetBlockDevSubsidy(pindexPrev->nHeight + 1, consensusParams);
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        ...
        if (txNew.vout.size() == 3 && ((pcoin.first->tx->vout[pcoin.second].scriptPubKey == scriptPubKeyKernel || pcoin.first->tx->vout[pcoin.second].scriptPubKey == aggregateScriptPubKeyHashKernel))
    ...
    {
        // Mine
        int64_t nReward = nTotalFees + GetBlockStakeSubsidy(pindexPrev->nHeight + 1, consensusParams);
        if (nReward < 0)
            return false;
    ...
    if (nCredit >= GetStakeSplitThreshold())
    {
        for(unsigned int i = 0; i < GetStakeSplitOutputs() - 1; i++)
            txNew.vout.push_back(CTxOut(0, txNew.vout[2].scriptPubKey)); //split stake
    }

    // Set output amount
    if (txNew.vout.size() == GetStakeSplitOutputs() + 2)
    {
        CAmount nValue = (nCredit / GetStakeSplitOutputs() / CENT) * CENT;
        for (unsigned int i = 2; i < GetStakeSplitOutputs() + 1; i++) {
            txNew.vout[i].nValue = nValue;
        }

        txNew.vout[GetStakeSplitOutputs() + 1].nValue = nCredit - nValue * (GetStakeSplitOutputs() - 1);
    }
    else
...
bool CWallet::CreateCoinStakeFromDelegate(interfaces::Chain::Lock& locked_chain, const FillableSigningProvider &keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, std::vector<COutPoint>& setDelegateCoins, std::vector<unsigned char>& vchPoD, COutPoint& headerPrevout)
{
    ...
    {
        // Delegate
        int64_t nTotalReward = nTotalFees + GetBlockStakeSubsidy(pindexPrev->nHeight + 1, consensusParams);
        if (nTotalReward < 0)
            return false;
    ...
    if(delegateOutputExist)
    {
        txNew.vout[3].nValue = nRewardOffline;
    }
...
void CWallet::StakeTalans(bool fStake, CConnman* connman)
{
    ::StakeTalans(fStake, this, connman, stakeThread);
}

void CWallet::StartStake(CConnman *connman)
{
    m_enabled_staking = true;
    StakeTalans(true, connman);
}
