...
inline void initState(){
    ...
    const std::string dirTalan = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<TalanState>(new TalanState(dev::u256(0), TalanState::openDB(dirTalan, hashDB, dev::WithExisting::Trust), dirTalan + "/talanDB", dev::eth::BaseState::Empty));
...
inline dev::Address createTalanAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline TalanTransaction createTalanTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    TalanTransaction txEth;
    if(recipient == dev::Address()){
        txEth = TalanTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = TalanTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<TalanTransaction> txs){
