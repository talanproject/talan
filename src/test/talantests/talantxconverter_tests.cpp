...
void checkResult(bool isCreation, std::vector<TalanTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    TalanTxConverter converter(transaction, NULL);
    ExtractTalanTX talanTx;
    BOOST_CHECK(converter.extractionTalanTransactions(talanTx));
    std::vector<TalanTransaction> result = talanTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(talantxconverter_tests, TestingSetup)
