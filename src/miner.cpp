...
void BlockAssembler::RebuildRefundTransaction(){
    ...
    if (pblock->IsProofOfStake()) {
        contrTx.vout[1].nValue = GetBlockDevSubsidy(nHeight, chainparams.GetConsensus());
        nSubsidy = GetBlockStakeSubsidy(nHeight, chainparams.GetConsensus());
    }

    contrTx.vout[refundvout].nValue = nFees + nSubsidy;
    contrTx.vout[refundvout].nValue -= bceResult.refundSender;
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    if (fProofOfStake) {
        ...
        coinstakeTx.vout[1].scriptPubKey = devScriptPubKey;
        coinstakeTx.vout[2].scriptPubKey = scriptPubKeyIn;
    ...
    globalSealEngine->setTalanSchedule(talanDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = talanDGP.getBlockSize(nHeight);
    minGasPrice = talanDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = talanDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    TalanTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractTalanTX resultConverter;
    if(!convert.extractionTalanTransactions(resultConverter)){
    ...
    dev::u256 txGas = 0;
    for(TalanTransaction talanTransaction : talanTransactions){
        txGas += talanTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + talanTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(talanTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, talanTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            talanDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = talanDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                talanDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                talanDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            talanDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            talanDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    TalanDelegation talanDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                talanDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = talanDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    talanDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    talanDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                talanDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(talanDelegations.GetDelegation(address, delegation) && TalanDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "talanstake";
            ...
            for(uint32_t i=beginningTime;i<beginningTime + MAX_STAKE_LOOKAHEAD;i+=STAKE_TIMESTAMP_MASK+1) {
                ...
                if (SignBlock(pblock, *pwallet, nTotalFees, i, setCoins, setDelegateCoins)) {
                    ...
                    std::unique_ptr<CBlockTemplate> pblocktemplatefilled(
                            BlockAssembler(Params()).CreateNewBlock(pblock->vtx[1]->vout[2].scriptPubKey, true, true, &nTotalFees,
...
void StakeTalans(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
