...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // talan
    uint256 hashUTXORoot; // talan
    ...
    void SetNull()
    {
        ...
        nBits          = 0;
        nNonce         = 0;
        hashStateRoot  = uint256(); // talan
        hashUTXORoot   = uint256(); // talan
    ...
    explicit CBlockIndex(const CBlockHeader& block)
    {
        ...
        nNonce         = block.nNonce;
        nMoneySupply   = 0;
        hashStateRoot  = block.hashStateRoot; // talan
        hashUTXORoot   = block.hashUTXORoot; // talan
        nStakeModifier = uint256();
        hashProof = uint256(); 
        prevoutStake   = block.prevoutStake; // talan
        vchBlockSigDlgt    = block.vchBlockSigDlgt; // talan
    }
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // talan
        block.hashUTXORoot   = hashUTXORoot; // talan
    ...
    bool IsProofOfWork() const // talan
    {
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // talan
        READWRITE(hashUTXORoot); // talan
        READWRITE(nStakeModifier);
        READWRITE(prevoutStake);
        READWRITE(hashProof);
        READWRITE(vchBlockSigDlgt); // talan
    }
