...
#include <talan/storageresults.h>


extern std::unique_ptr<TalanState> globalState;
...
using ExtractTalanTX = std::pair<std::vector<TalanTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // talan

//! -stakingminutxovalue default
static const CAmount DEFAULT_STAKING_MIN_UTXO_VALUE = 50 * COIN;
...
class TalanTxConverter{

public:

    TalanTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionTalanTransactions(ExtractTalanTX& talanTx);
...
private:
    ...
    TalanTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<TalanTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
