...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // talan
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // talan

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // talan
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // talan
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // talan
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // talan
...
UniValue transactionReceiptToJSON(const TalanTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination talanSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(talanSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&talanSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The talan address string"},
                },
                RPCResult{
            "{\n"
    ...
    TalanDelegation talanDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!talanDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = talanDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The talan address string for staker"},
                },
                RPCResult{
            "[{\n"
    ...
    if(!talanDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = talanDelegation.DelegationsFromEvents(events);
...
UniValue gettxout(const JSONRPCRequest& request)
{
            ...
            "     \"addresses\" : [          (array of string) array of talan addresses\n"
            "        \"address\"     (string) talan address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
