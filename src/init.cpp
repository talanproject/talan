 ...
 */
static const char* BITCOIN_PID_FILENAME = "taland.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("talan-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/talanproject/talan>";
    const std::string URL_WEBSITE = "<https://talanplatform.com>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateTalan");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path talanStateDir = GetDataDir() / "stateTalan";
                bool fStatus = fs::exists(talanStateDir);
                const std::string dirTalan(talanStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsTalanstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<TalanState>(new TalanState(dev::u256(0), TalanState::openDB(dirTalan, hashDB, dev::WithExisting::Trust), dirTalan, existsTalanstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(talanStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setTalanSchedule(talanDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
