...
#if defined(NDEBUG)
# error "Talan cannot be compiled without assertions."
#endif
...
std::unique_ptr<TalanState> globalState;
...
bool fAddressIndex = false; // talan
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        TalanDGP talanDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = talanDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = talanDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        TalanTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractTalanTX resultConverter;
        if(!converter.extractionTalanTransactions(resultConverter)){
        ...
        std::vector<TalanTransaction> talanTransactions = resultConverter.first;
        std::vector<EthTransactionParams> talanETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(const TalanTransaction& talanTransaction : talanTransactions){
            sumGas += talanTransaction.gas() * talanTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, talanTransaction.gasPrice());
            } else {
                txMinGasPrice = talanTransaction.gasPrice();
            }
            VersionVM v = talanTransaction.getVersion();
            if(v.format!=0)
            ...
            if(talanTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(talanTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(talanTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += talanTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)talanTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s TALAN additional fees, %d delta bytes\n",
...
{
    if (nHeight == 1) {
        return consensusParams.devFund;
    } else {
        CAmount nSubsidy = consensusParams.baseReward;
        int years = 23;

        for (int i = consensusParams.rewardEpoch; i <= nHeight; i += consensusParams.rewardEpoch) {
            if (i >= consensusParams.rewardEpoch * 23)
                break;

            nSubsidy -= nSubsidy * consensusParams.rewardEpochRate;
        }
...
CAmount GetBlockDevSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    return GetBlockSubsidy(nHeight, consensusParams) * 0.2;
}

CAmount GetBlockStakeSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    return GetBlockSubsidy(nHeight, consensusParams) * 0.8;
}
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // talan
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // talan
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit) {
    ...
    TalanDGP qtumDGP(globalState.get(), fGettingValuesDGP);
    ...
    TalanTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<TalanTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
bool CheckReward(const CBlock& block, CValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, CAmount nActualStakeReward, const std::vector<CTxOut>& vouts, CAmount nValueCoinPrev, bool delegateOutputExist)
{
    ...
    else
    {
        ...
        // Dev fund checks
        if (block.vtx[offset]->vout[1].scriptPubKey != devScriptPubKey) {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, error("CheckReward(): Dev fund payment is missing"), REJECT_INVALID, "bad-cs-dev-payment-missing");
        }

        if (block.vtx[offset]->vout[1].nValue < GetBlockDevSubsidy(nHeight, consensusParams))
        {
            return state.Invalid(ValidationInvalidReason::CONSENSUS, error("CheckReward(): Dev fund payment is invalid"), REJECT_INVALID, "bad-cs-dev-payment-invalid");
        }
        ...
        {
            CAmount nValueDefFund = block.vtx[offset]->vout[1].nValue;
            CAmount nReward = blockReward - gasRefunds - nValueDefFund;
            CAmount nValueStaker = block.vtx[offset]->vout[2].nValue;
            CAmount nValueDelegate = delegateOutputExist ? block.vtx[offset]->vout[3].nValue : 0;
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path talanDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(talanDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(talanDir.string(), std::ios::in | std::ios::out);
    ...
    for(TalanTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                TalanTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
...
bool TalanTxConverter::extractionTalanTransactions(ExtractTalanTX& talantx){
    ...
    std::vector<TalanTransaction> resultTX;
    ...
    talantx = std::make_pair(resultTX, resultETP);
    return true;
}

bool TalanTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool TalanTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
TalanTransaction TalanTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    TalanTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = TalanTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = TalanTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t TalanTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // talan
    TalanDGP talanDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setTalanSchedule(talanDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = talanDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = talanDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = talanDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // talan
...
///////////////////////////////////////////////////////////////////////////////////////// talan
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            TalanTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractTalanTX resultConvertTalanTX;
            if(!convert.extractionTalanTransactions(resultConvertTalanTX)){
            ...
            if(!CheckMinGasPrice(resultConvertTalanTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertTalanTX.first, blockGasLimit, pindex->pprev);
            ...
            for(TalanTransaction& qtx : resultConvertTalanTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertTalanTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertTalanTX.first[k].getNVout(),
                        resultConvertTalanTX.first[k].from(),
                        resultConvertTalanTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // talan
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // talan
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // talan
            globalState->setRootUTXO(oldHashUTXORoot); // talan
...
bool GetBlockPublicKey(const CBlock& block, std::vector<unsigned char>& vchPubKey)
{
    ...
    const CTxOut& txout = block.vtx[1]->vout[2];
...
bool GetBlockDelegation(const CBlock& block, const uint160& staker, uint160& address, uint8_t& fee)
{
    ...
    size_t minVoutSize = delegateOutputExist ? 4 : 3;
    ...
    CAmount nValueStaker = block.vtx[1]->vout[2].nValue - nValueCoin;
    CAmount nValueDelegate = delegateOutputExist ? block.vtx[1]->vout[3].nValue : 0;
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // talan
        uint32_t sizeBlockDGP = talanDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Talan PoS we need to sync this after every block to ensure txdb is populated for
