Gitian building
================
*Setup instructions for a Gitian build of Talan Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Talan
...
- [Building Talan Core](#building-talan-core)
...
Clone the git repositories for talan and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/talanproject/talan --recursive
git clone https://github.com/talanproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/talan/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the talan repository under 'Fetch and create inputs' to install sources which require
...
To build Talan Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the talan repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/talan/.git/
...
For example:
```bash
URL=https://github.com/laanwj/talan.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit talan=${COMMIT} --url talan=${URL} ../talan/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit talan=${COMMIT} --url talan=${URL} ../talan/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit talan=${COMMIT} --url talan=${URL} ../talan/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-talan:
./bin/gbuild --commit talan=${COMMIT},cpp-eth-talan=${ETHCOMMIT} --url talan=${URL},cpp-eth-talan=${ETHURL} ../talan/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the talan git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../talan/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/talanproject/talan-detached-sigs.git

BTCPATH=/some/root/path/talan
SIGPATH=/some/root/path/talan-detached-sigs

./bin/gbuild --url talan=${BTCPATH},signature=${SIGPATH} ../talan/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/talan-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/talan-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/talan-osx-build.assert
```
...
[talanproject/gitian.sigs](https://github.com/talanproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@www.talanplatform.com) and he will commit them.
