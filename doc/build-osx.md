...
1. Clone the Talan Core source code:
    ```shell
...
2.  Build Talan Core:
    ...
    Configure and build the headless Talan Core binaries as well as the GUI (if Qt is found).
...
## Running
Talan Core is now available at `./src/taland`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Talan"

touch "/Users/${USER}/Library/Application Support/Talan/talan.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Talan/talan.conf"
```

The first time you run taland, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Talan/debug.log
```

## Other commands:
```shell
./src/taland -daemon      # Starts the talan daemon.
./src/talan-cli --help    # Outputs a list of command-line options.
./src/talan-cli help      # Outputs a list of RPC commands when the daemon is running.
```
